...
```{r} 
# Calculate the number of routes passing through each bus stop
num_routes <- sapply(1:nrow(data_2022_sf), function(i) {
  bus_stop <- data_2022_sf[i,]
  intersected_routes <- st_intersects(routes_sf, bus_stop$geometry)
  sum(unlist(intersected_routes))
})

# Add the "num_routes" column to data_2022_sf
data_2022_sf$num_routes <- num_routes
```
...
data_joined <- st_join(data_2022_sf, routes_sf, join = st_intersects)

# Count the number of intersecting routes for each stop
route_counts <- data_joined %>%
  group_by('Stop ID') %>%
  summarise(routes_count = n_distinct(ROUTE_NUM))

# Merge the route counts back to the original data using st_join
data_2022_sf <- st_join(data_2022_sf, route_counts, by = "Stop ID")

```

```{r}
# Plot stops (data_2022_sf) and routes (routes_sf) using ggplot
stop_plot <- ggplot(data = data_2022_sf) +
  geom_sf(aes(color = "Stops")) +
  labs(color = "Data Type", title = "Stops Visualization")

route_plot <- ggplot(data = routes_sf) +
  geom_sf(aes(color = "Routes")) +
  labs(color = "Data Type", title = "Routes Visualization")

# Plot both on the same graph
combined_plot <- stop_plot +
  geom_sf(data = routes_sf, aes(color = "Routes")) +
  labs(color = "Data Type", title = "Combined Stops and Routes Visualization")
...
```{r} 
# Check CRS and transform if needed (assuming they are not already in the same CRS)
if (!identical(st_crs(data_2022_sf), st_crs(routes_sf))) {
  routes_sf <- st_transform(routes_sf, st_crs(data_2022_sf))
}

# Create an empty data frame to store results
result_df <- data.frame('Stop ID' = character(), nearest_routes = numeric(), stringsAsFactors = FALSE)

# Loop through each stop point and find nearest routes
for (i in 1:nrow(data_2022_sf)) {
  stop_point <- data_2022_sf[i, ]
  distances <- st_distance(stop_point, routes_sf, by_element = TRUE)
  nearest_routes <- sum(distances <= 0.01, na.rm = TRUE)  # Adjust the distance threshold as needed
  result_df <- rbind(result_df, data.frame('Stop ID' = stop_point$'Stop ID', nearest_routes = nearest_routes))
}
```
