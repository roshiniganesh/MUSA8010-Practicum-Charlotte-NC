...
train_data <- data_2022[train_index, ]
test_data <- data_2022[-train_index, ]

#Fit regression
OLS <- lm(Avg_ridership ~ TotalPop + MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5, data = train_data)

# Make prediction using testing data
OLS.testing <-
  test_data %>%
  mutate(Regression = "OLS Regression",
         Predictions = predict(OLS, test_data),
         Error = Predictions - Avg_ridership,
         AbsError = abs(Predictions - Avg_ridership),
         SalePrice.APE = (abs(Predictions - Avg_ridership)) / Predictions)
...
library(spgwr)
dependent_variable <- data_2022$Avg_ridership
independent_variables <- data_2022[, c("TotalPop", "MedHHInc", "MedRent", "Commutetowork","pctWhite", "pctAfricanamerican","pctAsian", "pctOther", "pctMale", "pctFemale", "pctBachelors", "pctWorker", "pctPoverty", "pctworkwithbus", "pctworkfromhome", "pctworkbycar", "pctworkbytaxi", "pctworkbymotor", "pctworkbybike",       "pctworkbywalk", "pctnocar", "pct1car", "pct2car", "pct3car", "pct4car", "pct5car", "policeoffice_nn1",   
"policeoffice_nn2", "policeoffice_nn3", "policeoffice_nn4", "policeoffice_nn5", "groceries_nn1",     
"groceries_nn2", "groceries_nn3", "groceries_nn4", "groceries_nn5", "parks_nn1", "parks_nn2", "parks_nn3",         "parks_nn4", "parks_nn5", "shoppingcen_nn1", "shoppingcen_nn2", "shoppingcen_nn3", "shoppingcen_nn4", "shoppingcen_nn5")]

coords <- st_coordinates(data_2022$geometry) %>%
  na.omit()
sp_points <- SpatialPoints(coords[, c("X", "Y")])%>%
  st_as_sf(data = data_2022)%>%
  na.omit()
data_2022 <- data_2022 %>%
  na.omit()
bandwidth <- 1000
# Fit GWR model
GWR <- gwr(formula = dependent_variable ~ independent_variables, data = data_2022, coords = sp_points, bandwidth = bandwidth) 
# Perform k-fold cross-validation
set.seed(123)  # Set seed for reproducibility
cv_results <- train(x = independent_variables, y = dependent_variable,
                    method = gwr_model,
...
# Evaluate the model
mse <- mean((test_data$Avg_ridership - predictions)^2)
