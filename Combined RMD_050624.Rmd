...
model_2022<-read_csv('New Data/data2022normalized.csv')
...
bus_data <- model_2022 %>% mutate(`Stop ID` = sprintf("%05s", `Stop ID`))
...
bus_stops.sf <- model_2022 %>%
  st_as_sf(coords = c("Longitude", "Latitude"), crs = "EPSG:3358") %>%
  distinct(`Stop ID`, .keep_all = TRUE)
...
ggplot(busstop_sf) +
  ...
  geom_sf(aes(color = '2023_Avg_Board'), size = .3) +
  scale_color_manual(low = "#FFAB00", high = "darkgreen") +
  mapTheme() +
...
train_index <- createDataPartition(data_2022$Avg_ridership_2022, p = 0.7, list = FALSE)
...
data_2022$'Stop ID' <- str_remove(data_2022$'Stop ID', "^0+")
...
#### Tunning hyperparameters 1

```{r}
# Split the data into training and testing sets
set.seed(123)
train_index2 <- sample(1:nrow(RF2_table_sf), 0.7 * nrow(RF2_table_sf))
train_data_RF2 <- RF2_table_sf[train_index2, ] %>% na.omit()
test_data_RF2 <- RF2_table_sf[-train_index2, ] %>% na.omit()

# Define the parameter grid for tuning only 'mtry'
tuneGrid <- expand.grid(
  mtry = c(10,20,30,40,50)  # Number of variables randomly sampled as candidates at each split
)
...
RF_model_tuned <- train(
  Avg_ridership_2022 ~ TotalPop + MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5+ total_routes_in_stops + total_stops_in_served_routes + PopDensity_km2 + transit_stops_nn1 + transit_stops_nn2 + transit_stops_nn3 + transit_stops_nn4 + transit_stops_nn5 + school_nn1 + school_nn2 + school_nn3 + school_nn4 + school_nn5 + dist_to_center,
  data = train_data_RF2,
  method = "rf",
  metric = "RMSE",
  tuneGrid = tuneGrid,
...
# Review the tuned model results
print(RF_model_tuned)

# Evaluate the model on the test set
predictions <- predict(RF_model_tuned, test_data_RF2)
results <- test_data_RF2 %>%
  mutate(
    Predictions_RF2 = predictions,
    Error_RF2 = Predictions_RF2 - Avg_ridership_2022,
    AbsError_RF2 = abs(Error_RF2),
    MAE_RF2 = mean(AbsError_RF2),
    APE_RF2 = (AbsError_RF2 / Avg_ridership_2022)
  ) %>%
  summarise(MAE = mean(AbsError_RF2), MAPE = mean(APE_RF2))
...
# Define the parameter grid for tuning only 'mtry'
tuneGrid <- expand.grid(
  mtry = c(3,6,9,12,15)  # Number of variables randomly sampled as candidates at each split
)
...
set.seed(123)
RF_model_tuned <- train(
  Avg_ridership_2022 ~ TotalPop + MedHHInc + pctAfricanamerican + pctFemale + pctBachelors + pctPoverty + pctworkwithbus + pctnocar + policeoffice_nn1 + groceries_nn3 + parks_nn2 + shoppingcen_nn1 + total_routes_in_stops + total_stops_in_served_routes + PopDensity_km2 + transit_stops_nn1 + school_nn1 + dist_to_center,
  data = train_data_RF2,
  method = "rf",
  metric = "RMSE",
  tuneGrid = tuneGrid,
...
```{r}
library(pROC)
roc_curve <- roc(test_data_RF2$Avg_ridership_2022, predicted2)
plot(roc_curve, main = "ROC Curve for Random Forest Model",
     col = "blue", lwd = 2, print.thres = TRUE)
auc_value <- auc(roc_curve)
text(0.6, 0.2, paste("AUC =", round(auc_value, 2)), col = "blue")
```
The ROC curve in the result appears to show a model that might not be performing well in terms of distinguishing between the classes for this Random Forest model. The AUC is 0.47, which falls below 0.5, suggesting this model performing worse than random guessing. This indicates that the model may be learning to predict the opposite of the actual classifications or there could be a significant issue with how the model or the data is set up.
