...
top_30_stops <- names(sort(underperformance_counts, decreasing = TRUE)[1:50])
underperforming_top_30 <- underperforming_stops[underperforming_stops$Stop_ID %in% top_30_stops, ]
underperforming_top_30 <- underperforming_top_30 %>%
  mutate(Total_Board_Alight = Board + Alight)

# Order
underperforming_top_30$Stop_ID <- factor(underperforming_top_30$Stop_ID, levels = rev(top_30_stops))

stacked_bar_plot <- ggplot(underperforming_top_30, aes(x = Stop_ID, fill = factor(Year))) +
...
underperforming_stops_2023 <- stops_data[(stops_data$Board + stops_data$Alight) <= 10 & stops_data$Year == 2022, ]
underperformance_counts_2023 <- table(underperforming_stops_2023$Stop_ID)

# Top 30 Stops for 2023
top_30_stops_2023 <- names(sort(underperformance_counts_2023, decreasing = TRUE)[1:50])
underperforming_top_30_2023 <- underperforming_stops_2023[underperforming_stops_2023$Stop_ID %in% top_30_stops_2023, ]


# Order
underperforming_top_30_2023$Stop_ID <- factor(underperforming_top_30_2023$Stop_ID, levels = rev(top_30_stops_2023))

bar_plot_top_30_2023 <- ggplot(underperforming_top_30_2023, aes(x = Stop_ID, fill = "2022")) +
...
map_plot_top_30_2023 <- ggplot() +
  ...
  scale_color_manual(values = rev(palettebase5),
                     ...
                     name = "Quintile\nBreaks") +
  labs(title = "Top 20 Bus Stops with Highest Underperformance Counts in 2022",
