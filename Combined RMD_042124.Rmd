...
count_stops_data <- separated_data %>%
  group_by(routes, `Stop ID`) %>%
...
RF2_table <- merge_count_data %>%
  group_by('Stop ID', Month) %>%
  summarize(
    routes = toString(unique(routes)),  # Merge routes separated by comma and space
    total_routes_in_stops = first(total_routes_in_stops),
    total_stops_in_routes = sum(total_stops_in_routes)
  )


RF2_sf <- st_as_sf(RF2_table, wkt = "geometry")
...
# Split the data into training and testing sets
set.seed(123)  # For reproducibility
train_index2 <- sample(1:nrow(RF2_table), 0.7 * nrow(RF2_table)) 
train_data_RF2 <- RF2_table[train_index2, ]
train_data_RF2 <- na.omit(train_data_RF2)
test_data_RF2 <- RF2_table[-train_index2, ]
test_data_RF2<- na.omit(test_data_RF2)


# Fit the Random Forest model
RF_model2 <- randomForest(
  formula = Avg_ridership ~ TotalPop + MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5+ total_of_routes + total_stops_in_served_routes, 
  data = train_data_RF2)
...
first_10_rows <- head(data_2022, 10)
first_10_rows<-as(first_10_rows,'Spatial')

# Select bandwidth for GWR
GWRbandwidth <- gwr.sel(Avg_ridership ~ TotalPop , data = first_10_rows, method = "aic", adapt = TRUE)

# Run GWR model
gwr_model <- gwr(Avg_ridership ~ TotalPop,
                 data = first_10_rows,
                 adapt = GWRbandwidth,
                 gweight = gwr.Gauss,
                 hatmatrix = TRUE,
...
```{r}
data_2022_sf <- st_as_sf(data_2022, wkt = "geometry")

# Extract x and y coordinates from the "geometry" column
data_2022$Longitude <- st_coordinates(data_2022_sf)[, "X"]
data_2022$Latitude <- st_coordinates(data_2022_sf)[, "Y"]
...
```{r}
data_2022_<-na.omit(data_2022_sf)
#data_2022 <- as(data_2022, 'Spatial')

# Select bandwidth for GWR
GWRbandwidth <- gwr.sel(Avg_ridership ~ TotalPop+ MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5, data = data_2022_sf, coords = c("Longitude", "Latitude"), method = "aic",adapt = TRUE)
# GWRbandwidth_fixed<-gwr.sel(formula=Avg_ridership ~TotalPop+ MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5, data=data_2022,method = "aic",adapt = FALSE)

# Run GWR model
gwr_model <- gwr(Avg_ridership ~ TotalPop+ MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct1car + pct2car + pct3car + pct4car + pct5car + policeoffice_nn1 + policeoffice_nn2 + policeoffice_nn3 + policeoffice_nn4 + policeoffice_nn5 + groceries_nn1 + groceries_nn2 + groceries_nn3 + groceries_nn4 + groceries_nn5 + parks_nn1 + parks_nn2 + parks_nn3 + parks_nn4 + parks_nn5 + shoppingcen_nn1 + shoppingcen_nn2 + shoppingcen_nn3 + shoppingcen_nn4 + shoppingcen_nn5,
                 data = data_2022_sf,
...
#### Reduced data size
```{r}
# Reduce data size and convert data to Spatial object
set.seed(123)  
sample_index <- sample(1:nrow(data_2022), 0.1 * nrow(data_2022)) 
sample_data <- data_2022[sample_index, ]
# List of percentage variables
percentage_vars <- c("pctWhite", "pctAfricanamerican", "pctAsian", "pctOther", "pctMale", "pctFemale", "pctBachelors", "pctWorker", "pctPoverty", "pctworkwithbus", "pctworkfromhome", "pctworkbycar", "pctworkbytaxi", "pctworkbymotor", "pctworkbybike", "pctworkbywalk", "pctnocar", "pct1car", "pct2car", "pct3car", "pct4car", "pct5car")

# Loop through each percentage variable and cap values at 1
for (var in percentage_vars) {
  sample_data[[var]] <- ifelse(sample_data$TotalPop > 0, pmin(sample_data[[var]] / sample_data$TotalPop, 1), 0)
}


sample_data <- as(sample_data, 'Spatial')


# Select bandwidth for GWR
GWRbandwidth <- gwr.sel(Avg_ridership ~ TotalPop + MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct2car + policeoffice_nn1 + groceries_nn1 + parks_nn1 + shoppingcen_nn1, data = sample_data, method = "aic", adapt = TRUE)

# Median imputation of missing values 
#sample_data$MedHHInc[is.na(sample_data$MedHHInc)] <- median(sample_data$MedHHInc, na.rm = TRUE)
#sample_data$MedRent[is.na(sample_data$MedRent)] <- median(sample_data$MedRent, na.rm = TRUE)

# Run GWR model 
gwr_model <- 
    gwr(Avg_ridership ~ TotalPop + MedHHInc + MedRent + Commutetowork + pctWhite + pctAfricanamerican + pctAsian + pctOther + pctMale + pctFemale + pctBachelors + pctWorker + pctPoverty + pctworkwithbus + pctworkfromhome + pctworkbycar + pctworkbytaxi + pctworkbymotor + pctworkbybike + pctworkbywalk + pctnocar + pct2car + policeoffice_nn1 + groceries_nn1 + parks_nn1 + shoppingcen_nn1,
        data = sample_data,
        adapt = GWRbandwidth,
        gweight = gwr.Gauss,
        hatmatrix = TRUE,
