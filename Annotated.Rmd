---
title: " Data Driven Framework to Correct Underperforming Transit Stops in Charlotte, NC"
...
### 7.Data Prep for Modeling

The following codes performs a detailed spatial analysis of bus stops within census tracts, using featuring engineering to calculates nearest neighbors for several facility types. Then merges datasets, visualizes stop density, and computes the distance to a central reference point. Each operation is designed to help having the data ready for our following modeling process.
...
data_2022 <- data_2022 %>% select(-year)
```
...
Once again, we created this Actual vs. Prediction plot, we can see the predictions are now close to the line of perfect prediction as well, which further indicates the fact that the performance of random forest model of bus routes is good.
...
ggplot(data_2022_scenario1_sf) +
  ...
  scale_color_gradient(low = "#FFBD37", high = "#005357") +
  mapTheme() +
  labs(title = "Bus Stops 2027", color = "Predicted Avg Ridership with \n15% Increase in Population Density")
...
We add 10 stops to the original dataset to see what willl happen when new stops are built. Again following the same step using Actual vs.Prediction sactterplot and map of absolute errors to visualize the performance, also mapping out the underperforming stops when 10 more stops are added.
...
# Perform a left join based on the common column "Stop ID"
merged_dataset_bus <- left_join( routes_df,joined_data_test, by = "Stop ID")
...
colnames(RF2_table)<-colnames(filtered_table2)  

table_test <- rbind(RF2_table_sf, filtered_table2)
```
...
```{r}
# Make Predictions with the New Scenario Data
predictions_add_stops <- predict(RF_model2, newdata = table_test)
...
#  scale_fill_gradient(low = "white", high = "grey20") + 
  ...
  scale_color_gradient(low = "#FFBD37", high = "#005357") +
  mapTheme() +
  labs(title = "Predicted Avg Ridership under 10 in January: Scenario VI", color = "Predicted Avg Ridership with \n10 New Added Stops")+
  theme(legend.position = "bottom")
```
